...
function autotrackDoTracking(data) {
    ...
    if (flags.wildmaps) {
        if (newbit(0x369, 0x20))
            toggle_map(0);

        if (newbit(0x369, 0x10))
            toggle_map(1);

        if (newbit(0x368, 0x20))
            toggle_map(2);

        if (newbit(0x369, 0x02))
            toggle_map(3);

        if (newbit(0x369, 0x04))
            toggle_map(4);

        if (newbit(0x368, 0x80))
            toggle_map(5);

        if (newbit(0x368, 0x10))
            toggle_map(6);

        if (newbit(0x368, 0x40))
            toggle_map(7);

        if (newbit(0x369, 0x01))
            toggle_map(8);

        if (newbit(0x368, 0x08))
            toggle_map(9);
    }

    if (flags.wildcompasses) {
        if (newbit(0x365, 0x20))
            toggle_compass(0);

        if (newbit(0x365, 0x10))
            toggle_compass(1);

        if (newbit(0x364, 0x20))
            toggle_compass(2);

        if (newbit(0x365, 0x02))
            toggle_compass(3);

        if (newbit(0x365, 0x04))
            toggle_compass(4);

        if (newbit(0x364, 0x80))
            toggle_compass(5);

        if (newbit(0x364, 0x10))
            toggle_compass(6);

        if (newbit(0x364, 0x40))
            toggle_compass(7);

        if (newbit(0x365, 0x01))
            toggle_compass(8);

        if (newbit(0x364, 0x08))
            toggle_compass(9);
    }
